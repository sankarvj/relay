...
func API(shutdown chan os.Signal, log *log.Logger, db *sqlx.DB, redisPool *redis.Pool, authenticator *auth.Authenticator, publisher *pubsub.Publisher) http.Handler {
	...
	app.Handle("POST", "/v1/accounts/:account_id/teams/:team_id/entities/:entity_id/templates", i.Create, mid.Authenticate(authenticator), mid.HasRole(auth.RoleAdmin, auth.RoleUser), mid.HasAccountAccess(db))
