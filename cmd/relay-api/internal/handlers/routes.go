...
func API(shutdown chan os.Signal, log *log.Logger, db *sqlx.DB, redisPool *redis.Pool, authenticator *auth.Authenticator, publisher *pubsub.Publisher) http.Handler {
	...
	app.Handle("GET", "/v1/accounts/:account_id/teams/:team_id/entities/:entity_id/items/templates", i.Templates, mid.Authenticate(authenticator), mid.HasRole(auth.RoleAdmin, auth.RoleUser), mid.HasAccountAccess(db))
