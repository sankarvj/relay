...
import (
	...
	"gitlab.com/vjsideprojects/relay/cmd/relay-slack-brain/internal/handlers"
...
func main() {
	if err := run(); err != nil {
		log.Println("slack brain main api error :", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	log := log.New(os.Stdout, "RELAY SLACK BRAIN API : ", log.LstdFlags|log.Lmicroseconds|log.Lshortfile)
	...
	var cfg struct {
		Web struct {
			APIHost         string        `conf:"default:0.0.0.0:3002,env:WEB_API_HOST"`
			DebugHost       string        `conf:"default:0.0.0.0:4002,env:WEB_DEBUG_HOST"`
		...
		DB struct {
			...
			Name       string `conf:"default:braindb,env:DB_NAME"`
		...
		SecDB struct {
			...
			Name     string `conf:"default:braindb,env:SEC_DB_NAME"`
		}
		PubSub struct {
		...
		Auth struct {
			...
			SlackSignature     string `conf:"default:79ca32ff75248bb57072deba8e20454e,env:SLACK_SIGNATURE"`
			SlackBotToken      string `conf:"default:xoxb-3020548349907-3931927797877-FFl8wEJCmnpI4RpDFXMCmewM,env:SLACK_BOT_TOKEN"`
		}
		Zipkin struct {
	...
	if err := conf.Parse(os.Args[1:], "BRAIN", &cfg); err != nil {
		if err == conf.ErrHelpWanted {
			usage, err := conf.Usage("BRAIN", &cfg)
			if err != nil {
	...
	authenticator, err := auth.NewSlackAuthenticator(privateKey, cfg.Auth.GoogleKeyFile, cfg.Auth.GoogleClientSecret, cfg.Auth.KeyID, cfg.Auth.Algorithm, f, cfg.Auth.SlackSignature, cfg.Auth.SlackBotToken)
	if err != nil {
	...
	rp := &redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", cfg.SecDB.Host, redis.DialPassword(cfg.SecDB.Password))
			if err != nil {
				return nil, err
			}
			return c, err
